...
era5 = Docpy.ERA5(list(ruta.glob('ERA5*'))[0])
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### Calculos

# Agarro los tiempos y las lat,lon
times_era, delta_t_era = era5.timedata()
every = 3
cada = int(every//delta_t_era)
times_wrf, delta_t_wrf = wrf_20km.timedata()

times = Docpy.functions.common_times([times_era,times_wrf])

times_era = times_era[times_era.index(times[0]):times_era.index(times[-1])+1]
times_wrf = times_wrf[times_wrf.index(times[0]):times_wrf.index(times[-1])+1]
idxs_wrfs = times_wrf.index(times[0]), times_wrf.index(times[-1])+1

lat, lon = era5.get_latlon()

# Agarro el z850 para cada coso

z850_20km = Docpy.campos.calc_z_lvl(wrf_20km, lvl=850)[idxs_wrfs[0]:idxs_wrfs[1]:cada,...]
z850_4A20 = Docpy.campos.calc_z_lvl(wrf_4A20, lvl=850)[idxs_wrfs[0]:idxs_wrfs[1]:cada,...]
z850_4B = Docpy.campos.calc_z_lvl(wrf_4B, lvl=850)[idxs_wrfs[0]:idxs_wrfs[1]:cada,...]
z850_HR3 = Docpy.campos.calc_z_lvl(wrf_HR3, lvl=850)[idxs_wrfs[0]:idxs_wrfs[1]:cada,...]
z850_era = Docpy.campos.calc_z_lvl(era5, lvl=850)[::cada,...]
...
anom_20km = z850_era - z850_20km
anom_4A20 = z850_era - z850_4A20
anom_4B = z850_era - z850_4B
anom_HR3 = z850_era - z850_HR3
diff_4A20_4B = z850_4A20 - z850_4B
...
for spinup in range(7):
    for ancho in range(1,11):
        ...
        ax2.plot(datetimes[spinup::2], rmse_inner20[spinup::2], label='20kmLR2')
        ax2.plot(datetimes[spinup::2], rmse_inner4A20[spinup::2], label='4n20')
        ax2.plot(datetimes[spinup::2], rmse_inner4B[spinup::2], label='4noNest')
        ax2.plot(datetimes[spinup::2], rmse_innerHR3[spinup::2], label='HR3', color='tab:red')
