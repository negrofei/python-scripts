...
max_mean_cicles = [np.ma.max(mean_cicle, axis=0) for mean_cicle in mean_cicles]

# Agarro el tiempo en el que se cumple que llega al maximo
time_max_pps = [np.ma.masked_equal(np.ones_like(max_mean_cicle),1) for max_mean_cicle in max_mean_cicles]

for m, mean_cicle in enumerate(mean_cicles):
    for t in range(0,mean_cicle.shape[0]):
        time_max_pps[m] = np.ma.where( mean_cicle[t]==max_mean_cicles[m], acum*t, time_max_pps[m])

# Promedio en caso que sea un ensemble

time_max_pp = np.ma.mean(
        np.ma.stack(
            time_max_pps,
            axis=0,
            ),
